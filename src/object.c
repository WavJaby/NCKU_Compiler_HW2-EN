...
const char* objectType2str[] = {
    ...
    [OBJECT_TYPE_ARRAY] = "list",
    [OBJECT_TYPE_SYMBOL] = "sym",
    [OBJECT_TYPE_FUNC] = "func",
    [OBJECT_TYPE_UNDEFINED] = "void",
};
...
Object object_nameLiteralOrLoadReg(const Object* src, char* buffer, const size_t bufferLen) {
    ...
    switch (src->type) {
    case OBJECT_TYPE_REGISTER:
        // TODO: (Week 2) register values don't need a load; write the register name directly into buffer and return *src
        //   see LLVM_IR_CHEATSHEET.md §Naming Conventions for the register naming format
        return (Object){OBJECT_TYPE_UNDEFINED, .value = {}};
    case OBJECT_TYPE_SYMBOL: {
        // TODO: (Week 4) handle three special SYMBOL cases, each with a different buffer format and return value
        //   1. capturedIndex >= 0: closure upvalue — loaded differently from a regular variable
        //   2. funcArg: function parameter — already in a register, no extra load needed
        //   3. type == OBJECT_TYPE_FUNC: function pointer — use the global symbol name
        //   see README.md §object_nameLiteralOrLoadReg for the IR operand format of each case

        // Load symbol value to register (normal variable)
...
bool object_packAsPtr(const ObjectType valType, const char* valName, char* buffer, const size_t bufferLen) {
    ...
    if (valType != OBJECT_TYPE_I64) {
        ...
        if (!op) {
            yyerrorf("unknown type '%s', cannot pack as pointer\n", objectType2str[valType]);
            return true;
        }
...
Object object_createNumber(const ScientificNotation* number) {
    if (number->type == ERROR) {
        yyerrorf("number literal could not be parsed\n");
        return (Object){OBJECT_TYPE_UNDEFINED, .value = {}};
    }
...
Object object_getIndex(const Object* obj, const Object* index, const YYLTYPE* objLoc, const YYLTYPE* indexLoc) {
    ...
    if (objType == OBJECT_TYPE_ARRAY) {
        if (obj->type != OBJECT_TYPE_SYMBOL || obj->value.symbol->elementType == OBJECT_TYPE_UNDEFINED) {
            yyerrorlf("array element type is unknown; cannot index\n", objLoc);
