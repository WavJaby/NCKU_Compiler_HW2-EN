...
bool code_stdoutPrintObject(Object* src, bool space, bool newLine) {
    ...
    case OBJECT_TYPE_STR: {
        // TODO: use buffPrintln to emit the printf call
        //   use objectType2llvmType[srcValueType] to get llvmType; build the format string @fmt_<type>[_n]
        //   example (I32 with newline): call i32 (ptr, ...) @printf(ptr @fmt_i32_n, i32 %reg0)
        //   see LLVM_IR_CHEATSHEET.md §Output Functions for format string naming rules
        break;
    }
    ...
    return false;
FAILED:
    yyerrorf("cannot print value of type '%s'\n", objectType2str[srcValueType]);
    if (src->type == OBJECT_TYPE_SYMBOL) object_free(&regSrc);
    object_free(src);
    return true;
}
...
// buffPrintln / compilerLog usage: see README.md §Utility Function Reference
bool code_createVariable(ValueData* valueData, char* name) {
    ...
    {
        // Quick Start: supports numeric literals only; uses sciToStr directly to get the LLVM operand string
        // TODO: once object_nameLiteralOrLoadReg is implemented, replace with:
        //   char regName[MAX_NAME_LENGTH];
        //   Object regSrc = object_nameLiteralOrLoadReg(src, regName, MAX_NAME_LENGTH);
        //   if (regSrc.type == OBJECT_TYPE_UNDEFINED) goto FAILED;
        //   ...  (use regName) ...
        //   if (src->type == OBJECT_TYPE_SYMBOL) object_free(&regSrc);
        // After switching, this will support: symbol references, booleans, strings, arrays, and register results from expressions
        char regName[MAX_NAME_LENGTH];
        char* numStr = sciToStr(src->value.number);
        snprintf(regName, MAX_NAME_LENGTH, "%s", numStr);
        free(numStr);

        // alloca: allocate stack space for %%var.N of size <type> (equivalent to declaring a local variable)
        // store:  write the initial value regName into the address pointed to by %%var.N
        // to read: %%regN = load <type>, ptr %%var.N  (handled by the SYMBOL branch of object_nameLiteralOrLoadReg)
...
bool code_assign(Object* dest, Object* src) {
    ...
    if (dest->type != OBJECT_TYPE_SYMBOL) {
        yyerrorf("assignment target must be a named variable, not '%s'\n", objectType2str[dest->type]);
        goto FAILED;
    }

    // TODO: implement variable assignment IR generation
    //   1. get the IR operand string for src (see object_nameLiteralOrLoadReg)
    //   2. verify src and dest types are compatible; report a semantic error if not
    //   3. emit a store IR instruction to write the value into dest's alloca address
    //   4. clean up Objects and return false
...
Object code_getLength(Object* obj, const YYLTYPE* loc) {
    ...
    if (objType != OBJECT_TYPE_ARRAY && objType != OBJECT_TYPE_STR) {
        yyerrorlf("'%s' is neither a list nor a string; cannot query its length\n", loc, objectType2str[objType]);
        object_free(obj);
        return (Object){.type = OBJECT_TYPE_UNDEFINED};
    }

    // TODO: get the object's LLVM operand, emit the length-query IR, and return a REGISTER Object
    //   1. get the IR operand string for obj (see object_nameLiteralOrLoadReg)
    //   2. allocate an I64 result register (see object_createRegisterSymbol)
...
bool code_arrayPush(const Object* arr, Object* val, const YYLTYPE* loc) {
    ...
    if (arr->type != OBJECT_TYPE_SYMBOL || arrType != OBJECT_TYPE_ARRAY) {
        yyerrorlf("'%s' is not a list; cannot push to it\n", loc, objectType2str[arrType]);
        goto FAILED;
    }
    ...
    if (arr->value.symbol->elementType == OBJECT_TYPE_UNDEFINED)
        ...
        yyerrorlf("list element type '%s' does not match pushed value type '%s'\n", loc,
...
int main(int argc, char* argv[]) {
    ...
    if (inputFilePath && !((yyin = fopen(inputFilePath, "rb")))) {
        fprintf(stderr, "error: '%s' not found or could not be opened\n", inputFilePath);
        return 1;
    }
