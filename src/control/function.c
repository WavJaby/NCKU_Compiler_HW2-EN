...
Object func_define(const ScientificNotation* funcDefCount, char* funcName) {
    ...
    if (count != 1) {
        yyerrorf("function count must be 1\n");
    ...
    if (!symbol) {
        yyerrorf("'%s' has already been declared\n", funcName);
        return (Object){OBJECT_TYPE_UNDEFINED, .value = {}};
    }
...
bool func_defineBodyEnd(Object* funcObj, char* funcName) {
    ...
    if (endFuncObj.type != OBJECT_TYPE_SYMBOL || endFuncObj.value.symbol->type != OBJECT_TYPE_FUNC) {
        yyerrorf("'%s' is not a function; cannot be used at function end\n", funcName);
        return true;
    }
...
bool code_return(const Object* obj, const YYLTYPE* tokenLoc) {
    ...
    ScopeData* func_scope = scope_getFunction();
    if (!func_scope) {
        yyerrorf("return statement must be inside a function\n");
        goto FAILED;
    }
    ...
    if (funcInfo->returnType != OBJECT_TYPE_UNDEFINED &&
        ...
        yyerrorf("return type is incompatible with previous return\n");
        goto FAILED;
    }
...
bool func_callArgAdd(FuncCallInfo* funcCall, Object* arg, const YYLTYPE* argLocation) {
    ...
    if (!currentNode) {
        yyerrorlf("wrong number of function arguments\n", argLocation);
        return true;
    }
    ...
    if (paramType != objType &&
        ...
        yyerrorlf("function argument type mismatch\n", argLocation);
        return true;
    }
...
void func_call(FuncCallInfo* funcCall, const Object* funcObj, ValueData* funcReturn, const YYLTYPE* tokenLoc) {
    ...
    if (funcInfo->builtin != NULL) {
        ...
        if (builtinVariant == -1) {
            yyerrorlf("function argument type mismatch (no matching variant for built-in function)\n", tokenLoc);
...
bool func_takeAndCall(const ScientificNotation* count, Object* funcObj, ValueData* valueData, const YYLTYPE* tokenLoc) {
    ...
    for (int32_t i = 0; i < argCount; i++) {
        ...
        if (!arg) {
            yyerrorlf("not enough arguments provided\n", tokenLoc);
            goto FAILED;
        }
