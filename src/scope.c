...
// cloneStruct / map_putpp / strdup usage: see README.md §Utility Function Reference
SymbolData* scope_addSymbol(const ObjectType type, char* name) {
    ...
    // TODO: before inserting, check all scopes for a duplicate symbol name; return NULL if found
    return symbolData;
}
...
Object scope_findSymbol(char* name) {
    for (CompilerContext* searchCtx = ctx; searchCtx; searchCtx = searchCtx->prev) {
        linkedList_foreach(&searchCtx->scopeStack, node) {
            ...
            // TODO: if searchCtx == ctx, the symbol is in the current context
            //   return a SYMBOL Object with capturedIndex = -1 (see the closure branch below for return format)
            if (searchCtx == ctx) {
                /* fill in */
    ...
    yyerrorf("'%s' has not been declared\n", name);
