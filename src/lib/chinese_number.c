...
#ifdef VERBOSE
            fprintf(stderr, "error: unrecognized character U+%04X\n", code[i]);
#endif
            free(tokens);
...
static uint8_t parse_tokens(const NumberToken* tokens, size_t token_count, ParseResult* result) {
    ...
    for (size_t i = token_count - 1; i > 0; --i) {
...
#ifdef VERBOSE
                fprintf(stderr, "error: negative sign must appear at the start\n");
#endif
                goto parse_error;
            }
        }
...
#ifdef VERBOSE
                fprintf(stderr, "error: multiplier cannot appear at the start of a number\n");
#endif
                goto parse_error;
            }
        ...
        case TOKEN_TYPE_DELIM: // 又 / 有
            if (digit_state == DIGIT_STATE_DELIM) {
                // 又又 is invalid
#ifdef VERBOSE
                fprintf(stderr, "error: delimiter '又' cannot appear consecutively\n");
#endif
                goto parse_error;
            }
        ...
        case TOKEN_TYPE_FRAC_MULT: // 分, 釐, ...
            if (digit_state == DIGIT_STATE_MULT) {
                // e.g. 微分 is invalid
#ifdef VERBOSE
                fprintf(stderr, "error: fractional-unit characters cannot be consecutive\n");
#endif
                goto parse_error;
            }
            mult_stack_clear(&stack);
        ...
        if (current_exp > result->exp) {
            ...
            if (!allow_fill) {
#ifdef VERBOSE
                fprintf(stderr, "error: missing multiplier between digits (gap %d -> %d)\n",
    ...
    if (result->count == 0) {
#ifdef VERBOSE
        fprintf(stderr, "error: no valid number found in input\n");
#endif
