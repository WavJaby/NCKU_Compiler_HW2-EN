...
static const char* yysymbolNameCh(yysymbol_kind_t symbol) {
    switch (symbol) {
    case YYSYMBOL_NUMBER_LIT: return " number ";
    case YYSYMBOL_STR_LIT: return " string ";
    case YYSYMBOL_VAR_TYPE: return " type ";
...
#define MESSAGE_MAX_LEN 1024
    ...
    if (lookahead != YYSYMBOL_YYEMPTY) {
        offset += snprintf(msg, MESSAGE_MAX_LEN, "unexpected %s", yysymbolNameCh(lookahead));
    }
    ...
    if (n > 0) {
        offset += snprintf(msg + offset, MESSAGE_MAX_LEN - offset, ", expected ");
        for (int i = 0; i < n; ++i) {
            if (i > 0) offset += snprintf(msg + offset, MESSAGE_MAX_LEN - offset, " or ");
